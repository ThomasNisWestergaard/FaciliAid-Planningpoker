...
export async function POST(request: NextRequest) {
  ...
  const del = await supabase.from("votes").delete().eq("round_id", roundRes.data.id);
  if (del.error) return NextResponse.json({ error: del.error.message }, { status: 500 });

  const update = await supabase.from("rounds").update({ is_revealed: false }).eq("id", roundRes.data.id);
